...
export default function EndingScreen() {
  ...
  const totalScore = ws ? (ws.message + ws.responsibility + ws.harmony) / 3 : 0;
  ...
  const getEpilogue = () => {
    ...
    if (totalScore >= 40)
      return 'بذلتَ ما استطعتَ. ربما كان بعض المسارات متعجّلاً، لكنك لم تتوقّف عن المحاولة. والأثر لا يُقاس بالكمال، بل بالاستمرار.';
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center p-6 overflow-y-auto z-40">
      ...
      >
        ...
        <p className="text-xs text-aether-300/60 mb-6">إليك ما تركتَه خلفك بعد {ws.dayCount} يوماً</p>

        <div className="grid grid-cols-2 gap-3 mb-6">
          <div className="glass-light rounded-xl p-3">
            <p className="text-2xl font-bold text-aether-400 glow-text">{Math.floor(ws.message)}</p>
            <p className="text-[10px] text-aether-300/60">الرسالة</p>
          </div>
          <div className="glass-light rounded-xl p-3">
            <p className="text-2xl font-bold text-impact-400 glow-teal">{Math.floor(ws.responsibility)}</p>
            <p className="text-[10px] text-aether-300/60">المسؤولية</p>
          </div>
          <div className="glass-light rounded-xl p-3">
            <p className="text-2xl font-bold text-ember-400 glow-amber">{Math.floor(ws.harmony)}</p>
            <p className="text-[10px] text-aether-300/60">الانسجام</p>
          </div>
          <div className="glass-light rounded-xl p-3">
            <p className="text-2xl font-bold text-impact-300">{ws.echoQuestsCompleted}</p>
            <p className="text-[10px] text-aether-300/60">أصداء مُطمئنة</p>
          </div>
        </div>
        ...
        <p className="mt-4 text-[10px] text-aether-300/40">
          {state.healProgress === 1 ? '✦ الأرض كلها اهتزّت حياةً ✦' : `${Math.round(state.healProgress * 100)}% من الأرض أحياها مرورك`}
        </p>
      </motion.div>
    </div>
  );
